...
public class Oblig3Test {
    ...
    void oppgave3() {
        ...
        // La inn noen flere verdier som tester scenarioer som ikke ble testa tidligere
        int[] a = {6, 14, 1, 8, 12, 3, 7, 9, 11, 13, 2, 5, 4, 18, 20, 19};
        for (int verdi : a) tre.leggInn(verdi);

        try {
            s = tre.toStringPostOrder();
            if (!s.equals("[2, 4, 5, 3, 1, 7, 9, 8, 6, 11, 13, 12, 19, 20, 18, 14, 10]")) {
                antallFeil++;
